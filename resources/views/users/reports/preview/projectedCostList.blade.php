    ...
    @foreach ($arr as $key =>  $item)
        ...
        <div class="card">
          <div class="card-header p-0 bg-dark" id="headingOne-{{ $key }}">
            <h2 class="mb-0">
              <button class="btn btn-block text-left text-white" style="font-size: 11px" type="button" data-toggle="collapse" data-target="#collapseOne-{{ $key }}" aria-controls="collapseOne-{{ $key }}">
                {{ ++$c.'. '.$item['name'] }}
              </button>
            </h2>
          </div>
          ...
          <div id="collapseOne-{{ $key }}" class="collapse" aria-labelledby="headingOne-{{ $key }}" data-parent="#accordionExample">
            <div class="card-body">
                <table id="" class="table table-sm table-striped table-bordered" style="font-size: 10px">
                    ...
                    @php
                        $sort = $item['result']->sortBy('suppliername');
